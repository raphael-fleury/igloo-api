...
import { findFollowingFeed } from "@/database/queries/post.queries";
import { CommandHandler } from "@/app/cqrs";

const DEFAULT_LIMIT = 10;
const MAX_LIMIT = 20;

interface GetFollowingFeedQuery extends PageQueryDto {
    profileId: string;
}

export class GetFollowingFeedHandler implements CommandHandler<GetFollowingFeedQuery, PostsPageDto> {
    constructor(private readonly postRepository: Repository<Post>) { }

    static get default() {
        return new GetFollowingFeedHandler(appDataSource.getRepository(Post));
    }

    async handle(query: GetFollowingFeedQuery) {
        const limit = Math.min(MAX_LIMIT, query.limit ?? DEFAULT_LIMIT);

        const qb = this.postRepository
            .createQueryBuilder("post")
            .apply(findFollowingFeed(query.profileId, query))
            .take(limit + 1);
