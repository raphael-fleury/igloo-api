...
import { findTrendingFeed } from "@/database/queries/post.queries";
import { CommandHandler } from "@/app/cqrs";

const DEFAULT_LIMIT = 10;
const MAX_LIMIT = 20;

export class GetTrendingFeedHandler implements CommandHandler<PageQueryDto, PostsPageDto> {
    constructor(private readonly postRepository: Repository<Post>) { }

    static get default() {
        return new GetTrendingFeedHandler(appDataSource.getRepository(Post));
    }

    async handle(query: PageQueryDto) {
        const limit = Math.min(MAX_LIMIT, query.limit ?? DEFAULT_LIMIT);

        const qb = this.postRepository
            .createQueryBuilder("post")
            .apply(findTrendingFeed(query))
            .take(limit + 1);
